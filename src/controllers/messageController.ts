...
function getCallerEmail(req: HybridRequest): string | null {
  return req.etudiant?.email ?? req.personnel?.email ?? null;
}

export const envoyerMessage = async (req: HybridRequest, res: Response) => {
  try {
    const expediteur = getCallerEmail(req);
    if (!expediteur) return res.status(401).json({ message: 'Non authentifié' });

    const { destinataire, contenu } = req.body as { destinataire?: string; contenu?: string };

    if (!destinataire || !contenu?.trim()) {
      return res.status(400).json({ message: 'destinataire et contenu sont requis' });
    }

    const destinataireExists =
      (await prisma.etudiant.findUnique({ where: { email: destinataire }, select: { id: true } })) ||
...
export const mesConversations = async (req: HybridRequest, res: Response) => {
  try {
...
export const getConversation = async (req: HybridRequest, res: Response) => {
  try {
    const email = getCallerEmail(req);
    if (!email) return res.status(401).json({ message: 'Non authentifié' });

    const { interlocuteur } = req.params as { interlocuteur: string };
...
export const compterNonLus = async (req: HybridRequest, res: Response) => {
  try {
    const email = getCallerEmail(req);
    if (!email) return res.status(401).json({ message: 'Non authentifié' });
...
export const marquerVu = async (req: HybridRequest, res: Response) => {
  try {
    const email = getCallerEmail(req);
    if (!email) return res.status(401).json({ message: 'Non authentifié' });
    ...
    if (msg.destinataire !== email) return res.status(403).json({ message: 'Accès refusé' });
