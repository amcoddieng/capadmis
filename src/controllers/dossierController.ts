...
export const changerStatus = async (req: Request & { personnel?: { id: number; role: string } }, res: Response) => {
  try {
    ...
    const VALID_STATUS: StatusDossier[] = ['non_demarre', 'EN_COURS_D_ETUDE', 'VALIDE', 'CHANGEMENT_A_APPORTER', 'DOCUMENT_MANQUANT'];
    ...
    const effectiveStatus: StatusDossier | undefined = status
    ...
    if (effectiveStatus) data.status = effectiveStatus;
...
async function verifierAccesChecklist(req: PersonnelRequest, dossierId: number) {
  const dossier = await prisma.dossier.findUnique({
    where: { id: dossierId },
  ...
  if (!estAdmin && !estAssigne) return { erreur: 'Accès refusé' };
  return { dossier };
}

export const obtenirChecklistDossier = async (req: PersonnelRequest, res: Response) => {
  try {
    ...
    if ('erreur' in acces) return res.status(acces.erreur === 'Dossier introuvable' ? 404 : 403).json({ message: acces.erreur });
...
export const modifierChecklistDossier = async (req: PersonnelRequest, res: Response) => {
  try {
    const id = Number(req.params['id']);
    if (!Number.isInteger(id)) return res.status(400).json({ message: 'ID invalide' });

    const { champ, valeur } = req.body as { champ?: string; valeur?: boolean };
    if (!champ || !CHECKLIST_FIELDS.includes(champ as ChecklistField) || typeof valeur !== 'boolean') {
    ...
    const index = CHECKLIST_FIELDS.indexOf(champ as ChecklistField);
    const valeurs = checklistActuelle as unknown as Record<ChecklistField, boolean>;
