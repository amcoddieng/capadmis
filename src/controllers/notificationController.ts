...
function getEmail(req: HybridRequest): string | null {
  return req.etudiant?.email ?? req.personnel?.email ?? null;
}

export const mesNotifications = async (req: HybridRequest, res: Response) => {
  try {
...
export const compterNonLues = async (req: HybridRequest, res: Response) => {
  try {
    const email = getEmail(req);
    if (!email) return res.status(401).json({ message: 'Non authentifié' });
...
export const marquerLue = async (req: HybridRequest, res: Response) => {
  try {
    const email = getEmail(req);
    if (!email) return res.status(401).json({ message: 'Non authentifié' });
    ...
    if (notif.destination !== email) return res.status(403).json({ message: 'Accès refusé' });
...
export const marquerToutesLues = async (req: HybridRequest, res: Response) => {
  try {
    const email = getEmail(req);
    if (!email) return res.status(401).json({ message: 'Non authentifié' });
