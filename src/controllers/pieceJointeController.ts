...
type CallerRole = 'etudiant' | 'admin_superadmin' | 'conseiller' | null;

async function resolveAccess(req: HybridRequest, codeDossier: string): Promise<{
  role: CallerRole;
  reason?: string;
}> {
  ...
  if (personnel) {
    ...
    if (!dossier) return { role: null, reason: 'Dossier introuvable' };

    const isAssigned =
      (personnel.role === 'conseiller_admission' && dossier.conseiller_admission_id === personnel.id) ||
      (personnel.role === 'conseiller_visa' && dossier.conseiller_visa_id === personnel.id);

    if (!isAssigned) return { role: null, reason: 'Vous n\'êtes pas assigné à ce dossier' };
...
function buildNom(codeDossier: string, type: string, etudiantId: number | undefined, originalName: string): string {
...
export const ajouterPieceJointe = async (req: MulterRequest, res: Response) => {
  try {
    ...
    if (role === 'conseiller') return res.status(403).json({ message: 'Les conseillers ne peuvent pas ajouter des pièces jointes' });
...
export const listerPiecesJointes = async (req: HybridRequest, res: Response) => {
  try {
...
export const getPieceJointe = async (req: HybridRequest, res: Response) => {
  try {
    const id = parseInt(req.params['id'] as string, 10);
    if (isNaN(id)) return res.status(400).json({ message: 'ID invalide' });

    const piece = await prisma.piece_jointe.findUnique({ where: { id }, select: PIECE_SELECT });
    if (!piece) return res.status(404).json({ message: 'Pièce jointe introuvable' });
    ...
    const url = await getSignedUrl(
      r2,
      new GetObjectCommand({ Bucket: R2_BUCKET, Key: piece.nom }),
      { expiresIn: 3600 }
    );
...
const TYPE_PJ_FOLDERS: Record<string, string> = {
...
export const modifierPieceJointe = async (req: MulterRequest, res: Response) => {
  try {
    const id = parseInt(req.params['id'] as string, 10);
    if (isNaN(id)) return res.status(400).json({ message: 'ID invalide' });
    ...
    const { role, reason } = await resolveAccess(req, piece.codeDossier);
    if (!role) return res.status(403).json({ message: reason });
    if (role === 'conseiller') return res.status(403).json({ message: 'Utilisez PATCH /:id/status pour modifier le statut' });
    ...
    const data: Record<string, unknown> = { nom: nouveauNom };

    if (role === 'admin_superadmin') {
      const { type, status } = req.body as { type?: TypePieceJointe; status?: StatusPieceJointe };
...
export const changerStatusPieceJointe = async (req: HybridRequest, res: Response) => {
  try {
    const id = parseInt(req.params['id'] as string, 10);
    if (isNaN(id)) return res.status(400).json({ message: 'ID invalide' });
    ...
    const { role, reason } = await resolveAccess(req, piece.codeDossier);
    if (!role) return res.status(403).json({ message: reason });
    if (role === 'etudiant') return res.status(403).json({ message: 'Les étudiants ne peuvent pas modifier le statut' });

    const { status } = req.body as { status: StatusPieceJointe };
...
export const supprimerPieceJointe = async (req: HybridRequest, res: Response) => {
  try {
    const id = parseInt(req.params['id'] as string, 10);
    if (isNaN(id)) return res.status(400).json({ message: 'ID invalide' });
    ...
    const { role, reason } = await resolveAccess(req, piece.codeDossier);
    if (!role) return res.status(403).json({ message: reason });
    if (role === 'conseiller') return res.status(403).json({ message: 'Les conseillers ne peuvent pas supprimer des pièces jointes' });
