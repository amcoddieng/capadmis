...
export const creerInfosDossier = async (req: HybridRequest, res: Response) => {
  try {
    const { code_dossier, niveau_etude, pays_souhaite, filieres, nombre_fois_bac, status } = req.body as {
      code_dossier: string;
      niveau_etude: string;
...
export const modifierInfosDossier = async (req: HybridRequest, res: Response) => {
  try {
    ...
    const { niveau_etude, pays_souhaite, filieres, nombre_fois_bac, status, code_validation } = req.body as {
      niveau_etude?: string;
      pays_souhaite?: string;
      filieres?: string[];
      nombre_fois_bac?: number;
