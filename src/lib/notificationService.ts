...
const MESSAGES: Record<TypeNotification, string> = {
...
const SUBJECTS: Record<TypeNotification, string> = {
...
const BADGES: Record<TypeNotification, string> = {
...
function infoBlock(label: string, value: string, accentColor: string, bgColor: string): string {
  return `
...
function buildDetails(type: TypeNotification, extras: NotifExtras): string {
  ...
  switch (type) {
    ...
    case 'assignation_conseiller_visa': {
      ...
      let html = '';
      if (prenomConseiller) html += infoBlock(role, `${prenomConseiller} ${nomConseiller ?? ''}`, '#1d4ed8', '#eff6ff');
      if (codeDossier) html += infoBlock('Numéro de Dossier', codeDossier, '#1d4ed8', '#eff6ff');
    ...
    case 'change_status': {
      let html = '';
      if (nouveauStatut) html += infoBlock('Nouveau Statut', nouveauStatut.replace(/_/g, ' '), '#d97706', '#fffbeb');
      if (codeDossier) html += infoBlock('Numéro de Dossier', codeDossier, '#1d4ed8', '#eff6ff');
...
function buildEmailHtml(type: TypeNotification, extras: NotifExtras): string {
