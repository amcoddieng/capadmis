...
async function validerCodeHelper(email, code, type) {
...
export const genererCode = async (req, res) => {
    try {
        const { email, type } = req.body;
        if (!email || !type) {
            return res.status(400).json({ message: 'email et type sont requis' });
        }
        const VALID_TYPES = ['modification_mot_de_passe', 'modification_infos'];
...
export const validerCode = async (req, res) => {
    try {
...
export const modifierMotDePasse = async (req, res) => {
    try {
        const { email, code, nouveau_mdp } = req.body;
        ...
        if (type_utilisateur === 'etudiant') {
            const etudiant = await prisma.etudiant.findUnique({ where: { email } });
            if (etudiant)
                await prisma.etudiant.update({ where: { id: etudiant.id }, data: { mdp: hashedMdp } });
        }
        else {
            const personnel = await prisma.personnel.findUnique({ where: { email } });
            if (personnel)
                await prisma.personnel.update({ where: { id: personnel.id }, data: { mdp: hashedMdp } });
        }
...
export const modifierInfos = async (req, res) => {
    try {
        const { email, code, ...infos } = req.body;
        if (!email || !code) {
        ...
        if (infos.nom)
            data.nom = infos.nom;
        if (infos.prenom)
            data.prenom = infos.prenom;
        if (infos.ville)
            data.ville = infos.ville;
        if (infos.payes)
            data.payes = infos.payes;
        if (infos.sexe)
            data.sexe = infos.sexe;
        if (infos.date_de_naissance)
        ...
        if (type_utilisateur === 'etudiant') {
            ...
            const personnelData = {};
            if (data.nom)
                personnelData.nom = data.nom;
            if (data.prenom)
                personnelData.prenom = data.prenom;
...
//# sourceMappingURL=codeTemporaireController.js.map